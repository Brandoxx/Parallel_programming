...
/**
 * @brief read arguments from the command line and intsialize the simulation
 * 
 * @return simulation_t* 
 */
int analyze_arguments(simulation_t* simulation, int argc, char* argv[]);
/**
 * @brief Create a consumers producers as a team of threads
 * 
 * @param simulation 
 * @return int 
 */
...
/**
 * @brief wait for the team of threads to finish
 * 
 * @param simulation 
 * @return int 
 */
int join_threads(size_t count, thread_data_t* private_data);
...
// we destroy in the opposite order of creation, if we create a mutex first
// we destroy it last
...
int simulation_run(simulation_t* simulation, int argc, char* argv[]) {
  ...
  if (error == EXIT_SUCCESS) {
    unsigned int seed = 0;
    // get random seed from the kernel
...
thread_data_t* create_threads(size_t count, void*(*subroutine)(void*), void* data) {
  // allocate memory for the threads
  pthread_t* threads = (pthread_t*) calloc(count, sizeof(pthread_t)); // allocate memory for the threads
  thread_data_t* private_data = (thread_data_t*) calloc(count, sizeof(thread_data_t));
  if (threads) { // if calloc succeeded
    for (size_t index = 0; index < count; ++index) {
      private_data[index].threadNumber = index;
      private_data[index].simulation = (simulation_t *) data;
      private_data[index].totalThreads = count;
      if (pthread_create(&threads[index], /*attr*/ NULL, subroutine, (void*) &private_data[index])
         == EXIT_SUCCESS) {
      } else { //defensive programming in case there are too many threads
...
int join_threads(size_t count, thread_data_t* private_data) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    error += pthread_join(private_data[index].self, /*value_ptr*/ NULL);
  }
  free(private_data);
  return error;
}
