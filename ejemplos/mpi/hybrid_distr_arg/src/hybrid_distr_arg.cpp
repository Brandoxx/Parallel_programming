...
int main(int argc, char* argv[]) {
  if (MPI_Init(&argc, &argv) == MPI_SUCCESS) {  // Initialize MPI
    ...
    if (argc == 3) {  // Check if thread count was specified
      ...
      const int process_size = process_finish - process_start;  // Calculate process size
      ...
      #pragma omp parallel default(none) \
        shared(std::cout, process_hostname, process_number)
      {
        ...
        const int thread_size = thread_finish - thread_start;  // Calculate thread size
...
int calculate_start(int rank, int end, int workers, int begin) {
  ...
  return begin + rank * (range / workers) + std::min(rank , range % workers);  // Calculate start
}
