 ...
 * @param data Contains a thread number and a pointer to a private data
 * @return Nothing.
 */
        ...
        if (error == EXIT_SUCCESS) {
            ...
            if (error == EXIT_SUCCESS) {
                ...
                if (error == EXIT_SUCCESS) {
                    ...
                    uint64_t time = (end.tv_sec - start.tv_sec) * 1e9;
                    printf("Time: %" PRIu64 " nanoseconds\n", time/1000000);
